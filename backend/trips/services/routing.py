...
class RoutingService:
    ...
    def calculate_required_stops(
        ...
        remaining_cycle_hours = MAX_DRIVING_TIME_CYCLE - current_cycle_hours
        cycle_hours_used_this_trip = 0.0
        ...
        if remaining_cycle_hours <= 0:
            ...
            order_counter += 1
            current_time += timedelta(hours=CYCLE_RESET_HOURS)
            remaining_cycle_hours = MAX_DRIVING_TIME_CYCLE
            cycle_hours_used_this_trip = 0.0
        ...
        for leg_index, leg in enumerate(legs):
            ...
            if (
                ...
                for idx in range(len(leg_steps_distance)):
                    ...
                    if cycle_hours_used_this_trip + step_hours >= remaining_cycle_hours:
                        arrival_time = current_time + timedelta(seconds=step_time)
                        
                        stops.append({
                            "type": "cycle_reset",
                            "coordinates": step_coords,
                            "timestamp": arrival_time.isoformat() + "Z",  # When they arrive at the stop
                            "daily_driving": 0.0,
                            "daily_duty": 0.0,
                            "weekly_driving": 0.0,
                        ...
                        current_time = arrival_time + timedelta(hours=CYCLE_RESET_HOURS)
                        daily_driving_seconds = 0.0
                        daily_duty_seconds = 0.0
                        remaining_cycle_hours = MAX_DRIVING_TIME_CYCLE
                        cycle_hours_used_this_trip = 0.0
                    ...
                    elif current_distance + step_distance >= MAX_FUEL_RANGE_MILES:
                        arrival_time = current_time + timedelta(seconds=step_time)

                        stops.append({
                            "type": "fuel",
                            "coordinates": step_coords,
                            "timestamp": arrival_time.isoformat() + "Z",  # When they arrive at fuel stop
                            "daily_driving": (daily_driving_seconds + step_time)/3600,
                            "daily_duty": (daily_duty_seconds + step_time + FUEL_STOP_TIME*3600)/3600,
                            "weekly_driving": current_cycle_hours + cycle_hours_used_this_trip + step_hours,
                            "distance_traveled": current_distance + step_distance,
                            "order": order_counter,
                            "duration_hours": FUEL_STOP_TIME
                        })
                        order_counter += 1

                        current_distance = 0.0
                        current_time = arrival_time + timedelta(hours=FUEL_STOP_TIME)
                        daily_duty_seconds += step_time + FUEL_STOP_TIME * 3600
                        daily_driving_seconds += step_time
                        cycle_hours_used_this_trip += step_hours
                        stop_added = True

                    elif daily_driving_seconds + step_time >= MAX_DRIVING_HOURS * 3600:
                        arrival_time = current_time + timedelta(seconds=step_time)
                        ...
                        stops.append({
                            ...
                            "distance_traveled": current_distance + step_distance,
                            "order": order_counter,
                            "duration_hours": REQUIRED_BREAK_HOURS
                        })
                        order_counter += 1

                        current_time = arrival_time + timedelta(hours=REQUIRED_BREAK_HOURS)
                        daily_driving_seconds = 0.0
                        daily_duty_seconds = 0.0
                        cycle_hours_used_this_trip += step_hours
                        stop_added = True
                        
                    elif daily_duty_seconds + step_time >= MAX_DUTY_WINDOW_HOURS * 3600:
                        arrival_time = current_time + timedelta(seconds=step_time)
            ...
            stops.append({
                ...
                "timestamp": arrival_time.isoformat() + "Z",
                "daily_driving": daily_driving_seconds/3600,
                "daily_duty": (daily_duty_seconds + PICKUP_DROPOFF_TIME*3600)/3600,
                "weekly_driving": current_cycle_hours + cycle_hours_used_this_trip,
                "distance_traveled": current_distance,
                "order": order_counter
            })
            order_counter += 1

            current_time = arrival_time + timedelta(hours=PICKUP_DROPOFF_TIME)
